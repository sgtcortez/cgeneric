...
stack* stack_construct(uint16_t capacity, uint16_t size_per_element, char *typename)
{
    const size_t required_size = sizeof(stack) + capacity * size_per_element;
    stack* q = malloc(required_size);
    stack_init(q, capacity, size_per_element, typename);
    return q;
}

void stack_init(stack *stack, uint16_t capacity, uint16_t size_per_element, char* typename)
{
...
bool stack_push(stack* stack, const void* const data, const char* typename)
{
...
void stack_pop(stack* stack, void* result, const char* typename)
{
...
uint16_t stack_size(const stack* const stack)
{
    return stack->top;
}

void stack_resize(stack** stack, uint16_t capacity)
{   
    struct stack* new_stack = stack_construct(capacity, (*stack)->size_per_element, NULL);
...
void stack_peek(stack* stack, void* result, const char* typename)
{
...
void stack_for_each(stack* stack, void (*callback)(void* data, void* user_data), void* user_data, const char* typename)
{
    assert(stack->hash == hash_djb2(typename) && "Stack typenames mismatch");
